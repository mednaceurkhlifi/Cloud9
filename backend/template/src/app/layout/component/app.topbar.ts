...
import { WorkspaceProjection } from '../../services/models/workspace-projection';
...
@Component({
    selector: 'app-topbar',
    standalone: true,
    imports: [RouterModule, CommonModule, StyleClassModule, AppConfigurator, Avatar, Button, Toast, Menu, Ripple],
            ...
            <div class="layout-topbar-actions">
                ...
                <div class="layout-topbar-menu hidden lg:block">
                    <div class="layout-topbar-menu-content">
                        <a type="button" class="layout-topbar-action" routerLink="/workspace/profile">
                        ...
                        <button type="button" class="layout-topbar-action" (click)="menu.toggle($event)" pRipple>
            ...
            <ng-template *ngIf="isMessage" let-message #message>
                <div class="flex flex-col items-start flex-auto">
                    <div class="flex items-center gap-2">
                        <p-avatar *ngIf="this.comingMessage.sender?.image == 'default_user.jpg'" image="default_user.jpg" shape="circle" />
                        <p-avatar *ngIf="this.comingMessage.sender?.image != 'default_user.jpg'" image="http://localhost:8082/api/v1/project-document/images/{{ this.comingMessage.sender?.image }}" shape="circle" />
...
export class AppTopbar implements OnInit {
    ...
    ngOnInit() {
        ...
        this._wkService
            ...
            .subscribe({
                next: (response) => {
                    this.workspace = response.projection!;
                },
                error: (err) => {
                    // treat errors
                }
        ...
        this.tasks.forEach((task) => {
            if (task.module) {
                if (this.modules.findIndex((m) => m.moduleId == task.module?.moduleId) == -1) {
                    this.modules.push(task.module);
                }
            }
        });
        ...
        this.socketClient.connect({}, () => {
            ...
            this.socketClient.subscribe(topic, (message: any) => {
                ...
                this.roomName = this.workspace.name!;
            ...
            this.projects.forEach((project) => {
                ...
                this.socketClient.subscribe(topic, (message: any) => {
                    ...
                    this.roomName = project.project?.name!;
                    if (this.comingMessage.sender?.email != this.user_email) {
                        if (!this.visible) {
                            this.messageService.add({ key: 'confirm', sticky: true, severity: 'success', summary: this.comingMessage.message });
                            this.visible = true;
    ...
    onConfirm() {
        if (this.isMessage) {
            if (this.getTargetName() == 'workspace') this.router.navigate(['/workspace']);
            else {
